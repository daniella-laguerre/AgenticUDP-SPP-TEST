...
is the server `.exe`. The bench client also changed in v2.15.0 — see
the note below.
...
> **v2.15.0 (2026-05-01): both the bench client and the transport changed.**
>
> The bench binary (`eo-ingest-bench-windows-amd64.exe`) ships three
> correctness fixes that affect what the UDP numbers mean:
>
> 1. **Inflight key wraparound fixed.** Sessions with more than 65,535
>    sends silently overwrote inflight entries, causing false ACKs and
>    corrupt retransmit records. Fixed with a monotonic `uint32` map key.
>    Affects any n=10,000 run at 5000 spans/iter (total sends >> 65,535).
>
> 2. **`acked` overcounting fixed.** Duplicate server ACKs were
>    incrementing `acked` even when the entry was already removed.
>    This caused the UDP drain condition to fire early, making
>    `WallClockSeconds` shorter than actual delivery time and inflating
>    `spans/sec_wall`. Fixed: `acked` only increments on found-and-removed.
>
> 3. **`spans/sec_wall` is now the primary UDP throughput metric.**
>    `p50_ms` for UDP now reflects **client send latency** (how long
>    `SendCycle` blocks, including backpressure from the inflight window)
>    rather than end-to-end ACK round-trip. `spans/sec_wall` =
>    `(iters × spans_per_call) / WallClockSeconds` covers the full
>    send→ACK→drain cycle and is the honest system-level number.
>    Previous UDP `p50_ms` was serialized per-iteration ACK-wait, making
>    UDP look slower than HTTP/gRPC even when the protocol was faster.
>
> **Refresh `eo-ingest-bench-<platform>.exe` and
> `eo-bench-merge-<platform>.exe` before running the n=10,000 suite:**

> **SHA-256 to verify after copying to the VM:**
> ```
...
> ```
> ```powershell
> $expected = "343c865658641ececdc18d2fb995763e99af200f51f509010881929da0436cdf"
> $actual   = (Get-FileHash C:\entropyops\eo-ingest-bench-windows-amd64.exe -Algorithm SHA256).Hash.ToLower()
> if ($actual -eq $expected) { Write-Host "OK" -ForegroundColor Green } else { Write-Host "MISMATCH — do not run" -ForegroundColor Red }
> ```
...
Set-Location C:\entropyops
Unblock-File .\eo-ingest-bench-windows-amd64.zip
tar -xf .\eo-ingest-bench-windows-amd64.zip -C .
Copy-Item "$env:USERPROFILE\Downloads\eo-bench-merge-windows-amd64.zip" C:\entropyops\ -Force
Unblock-File .\eo-bench-merge-windows-amd64.zip
tar -xf .\eo-bench-merge-windows-amd64.zip -C .
```
...
> JSON files written by old bench builds remain readable by the new tool
> and merger — the new fields are `omitempty` and decode as zero-valued
> for old chunks. But mixed old+new chunks in a single merge will produce
> a `spans/sec_wall` total that excludes old-format chunks (they didn't
> record `WallClockSeconds`), so re-run any chunked sweep already in
> progress under the new binary if you need the wall-clock numbers to be
> cited honestly.
...
Use this only if you have a multi-day uninterrupted host (no idle
disconnect, no auto-stop, no patch reboots). **This is the v2.15.0
canonical command** — verify the binary SHA before running (see §1d).

```powershell
# Verify binary first
$expected = "343c865658641ececdc18d2fb995763e99af200f51f509010881929da0436cdf"
$actual   = (Get-FileHash C:\entropyops\eo-ingest-bench-windows-amd64.exe -Algorithm SHA256).Hash.ToLower()
if ($actual -ne $expected) { Write-Host "MISMATCH — do not run" -ForegroundColor Red; exit 1 }
...
.\eo-ingest-bench-windows-amd64.exe `
  ...
  -warmup 50 `
  -udp-ack-timeout-ms 30000 `
  -json   bench-5000-n10000-run1.json
```

Accept criteria (v2.15.0): `fail%` = 0.0 for UDP, `spans/sec_wall`
UDP > HTTP and gRPC, `retransmits` = 0 or near-zero.
...
| Column | Meaning |
|--------|---------|
| `p50/p90/p99 ms` | Client-side latency per call. **For UDP (v2.15.0+): this is client send latency** — how long `SendCycle` blocks including inflight backpressure — not end-to-end ACK round-trip. |
| `ms/span` | `p50_ms / spans_per_call` — cross-batch-size comparable. |
| `spans/sec_avg` | Throughput computed from the per-iteration average. Useful for comparing batch sizes but not for cross-path UDP-vs-HTTP comparisons (UDP send and ACK are decoupled). |
| `spans/sec_wall` | **Primary UDP metric (v2.15.0+).** `(iters × spans) / WallClockSeconds`. `WallClockSeconds` covers the full window from first send to last ACK confirmed. This is the honest system-level throughput number — use it for cross-path comparisons. |
| `wall_s` | Total wall-clock seconds for the measurement window (send + ACK drain). |
| `fail%` | Percentage of iterations (HTTP/gRPC) or datagrams (UDP) that failed. **Must be 0.0 for a publishable UDP run.** Non-zero means datagrams were unACKed after the drain timeout. |
| `retx` (UDP only) | AgenticUDP retransmits. Should be `0` on a clean run. Non-zero means receive buffer pressure or coalescer pressure. |
| `drops` (UDP only) | Server kernel drops. Should be `0`. Non-zero: raise `ENTROPYOPS_AGENTIC_UDP_RCVBUF_BYTES`. |

**Accept criteria for a v2.15.0 UDP run to be publishable:**

| Check | Required |
|---|---|
| `fail%` | `0.0` — all datagrams ACKed within drain timeout |
| `retransmits` | `0` or near-zero — backoff fix held |
| `spans/sec_wall` UDP > HTTP | UDP system throughput higher than HTTP |
| `spans/sec_wall` UDP > gRPC | UDP system throughput higher than gRPC |
...
| `fail% > 0` on a UDP run (v2.15.0 binary) | Datagrams went unACKed past the drain timeout — either the `acked` counter advanced past actual ACKs (old binary) or the server dropped / didn't ACK all datagrams | First confirm you're running the v2.15.0 binary (SHA check in §1d). If SHA is correct, raise `-udp-ack-timeout-ms` (try 60000) and re-run. If `fail%` > 0 persists, share `retx`, `drops`, and server SLOW FLUSH lines. |
| UDP `spans/sec_wall` looks the same as old `spans/sec` | You're running a pre-v2.15.0 bench binary that doesn't output `WallClockSeconds` | Refresh the bench client (§1d) and re-run. Old binary `spans/sec` is per-iteration average, not wall-clock. |
| `spans/sec_wall` lower than expected after v2.15.0 upgrade | Expected — the old binary overcounted `acked` and ended the drain early, inflating `spans/sec`. The new binary waits for all ACKs. | This is the correct number. Compare against the accept criteria in §5 rather than the pre-v2.15.0 baseline. |
